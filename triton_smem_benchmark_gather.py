...
def triton_local_gather(base_ptr, vec_ptr, ts_0_ptrs, ts_1_ptrs, out_ptr1, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    for _ in range(0, gpu_iters):
        ...
        base_ptr_smem = tl.load(base_block, None, eviction_policy='evict_first')
...
def triton_gemv_0(base, vec, ts_0, ts_1, benchmark: bool = False):
    ...
    with torch.cuda._DeviceGuard(0):
        ...
        if benchmark:
            ...
            print(f"Elapsed gather kernel time for {benchmark_iters} iterations, time:\n{times}")
            print(f"Sync Times (excludes launch overhead):\n{sync_times}")
        else:
...
# Print here to indicate the script started, even though GPU calls may hang
import sys
