...
#include "third_party/nano/include/Analysis/RangeAnalysis.h"
...
struct TestNANORangeAnalysisPass
    : PassWrapper<TestNANORangeAnalysisPass, OperationPass<ModuleOp>> {

  MLIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(TestNANORangeAnalysisPass)
  ...
  void runOnOperation() override {
    ...
    DenseMap<Value, SetVector<Operation *>> assumptions =
        NANO::TritonIntegerRangeAnalysis::collectAssumptions(getOperation());
    std::shared_ptr<DataFlowSolver> solver = createDataFlowSolver();
    NANO::TritonIntegerRangeAnalysis *rangeAnalysis =
        solver->load<NANO::TritonIntegerRangeAnalysis>(
            assumptions, &getAnalysis<DominanceInfo>());
    NANO::initializeFuncOps(mod, rangeAnalysis);
    ...
    auto nonNegativePred = [&solver](Value v) -> bool {
      if (const auto *r =
        ...
        if (r->getValue().isUninitialized())
          return false;
        if (NANO::isEmptyInitializedRange(r->getValue().getValue()))
          return false;
      }
    ...
    mod.walk<WalkOrder::PreOrder>([&solver](FuncOp funcOp) {
      ...
      if (auto argRanges = NANO::collectRanges(*solver, args)) {
        int i = -1;
    ...
    mod->walk<WalkOrder::PreOrder>([&solver, nonNegativePred,
      ...
      if (auto outputRanges = NANO::collectRanges(*solver, results)) {
        int i = -1;
        ...
        if (auto cmpOp = llvm::dyn_cast<arith::CmpIOp>(op)) {
          if (NANO::cmpIIsStaticallyTrue(*solver, cmpOp))
...
namespace mlir::test {
void registerTestTritonNANOGPURangeAnalysis() {
  PassRegistration<TestNANORangeAnalysisPass>();
}
} // namespace mlir::test
