...
class ConvertMaskedLoadOp
    : public OpRewritePattern<triton::nanogpu::MaskedLoadOp> {
public:
  ConvertMaskedLoadOp(MLIRContext *context, const NANO::TargetInfo &targetInfo)
      : OpRewritePattern(context), targetInfo(targetInfo) {}

  LogicalResult matchAndRewrite(triton::nanogpu::MaskedLoadOp loadOp,
    ...
    std::tie(volatileFlag, nonTmpFlag) =
        mlir::LLVM::NANO::getCacheModifierFlagsForLoadStore(
            cacheMod, mlir::LLVM::NANO::MemoryOp::Load);
    ...
    auto createLoadWithAttrs = [&](Location loadLoc) -> Value {
      ...
      if (multicastMask && targetInfo.supportsClusterLoadBitWidth(vecBits)) {
        ...
        auto cacheModBits = LLVM::NANO::getCtrlBitsForCacheModifierOnTarget(
      ...
      if (loadOp.getForceNoAlias()) {
        NANO::addLocalLoadNoAliasScope(load);
      }
      return load;
    };
...
class ConvertMaskedStoreOp
    : public OpRewritePattern<triton::nanogpu::MaskedStoreOp> {
public:
  using OpRewritePattern::OpRewritePattern;

  LogicalResult matchAndRewrite(triton::nanogpu::MaskedStoreOp storeOp,
                                PatternRewriter &rewriter) const override {
    ...
    std::tie(volatileFlag, nonTmpFlag) =
        mlir::LLVM::NANO::getCacheModifierFlagsForLoadStore(
            storeOp.getCache(), mlir::LLVM::NANO::MemoryOp::Store);
    ...
    auto createStoreWithAttrs = [&](Location storeLoc) -> LLVM::StoreOp {
      ...
      if (storeOp.getForceNoAlias()) {
        NANO::addLocalLoadNoAliasScope(store);
      }
      return store;
    };
