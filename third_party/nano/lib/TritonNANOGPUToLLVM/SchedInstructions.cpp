...
using ::mlir::LLVM::NANO::isChainDotHead;
...
struct InstructionSchedHintsRewriter
    : public OpRewritePattern<triton::nanogpu::InstructionSchedHint> {
  ...
  LogicalResult
  matchAndRewrite(triton::nanogpu::InstructionSchedHint instructionSchedHint,
                  PatternRewriter &rewriter) const override {
    auto schedVariant = instructionSchedHint.getVariant();
    if (schedVariant == mlir::triton::nanogpu::SchedHint::none) {
    ...
    const bool limitSchedulingRange =
        schedVariant == mlir::triton::nanogpu::SchedHint::attention;
    ;
    ...
    switch (schedVariant) {
    case mlir::triton::nanogpu::SchedHint::attention:
      createIglpOpt(rewriter, loc, 2);
      break;
    case mlir::triton::nanogpu::SchedHint::none:
    default:
      break;
    }
...
struct TritonNANOGPULowerInstructionSchedHints
  ...
  void runOnOperation() override {
    ...
    target.addIllegalOp<triton::nanogpu::InstructionSchedHint>();
...
struct TritonNANOGPUInsertInstructionSchedHints
  ...
  void runOnOperation() override {
    ...
    auto schedHint = mlir::triton::nanogpu::SchedHint::none;
    std::transform(variant.begin(), variant.end(), variant.begin(),
                   [](unsigned char c) { return std::tolower(c); });
    if (auto maybeSchedHint = triton::nanogpu::symbolizeSchedHint(variant))
    ...
    case mlir::triton::nanogpu::SchedHint::attention:
      mod.walk([&](scf::ForOp forOp) {
        ...
        if (result.wasInterrupted()) {
          ...
          triton::nanogpu::InstructionSchedHint::create(
