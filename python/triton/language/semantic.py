...
def atomic_load(ptr: tl.tensor, mask: Optional[tl.tensor], other: Optional[tl.tensor], sem: str, scope: str,
    ...
    print(f"what is the type of the load: {ptr.type}")
    if ptr.type.is_ptr() and ptr.type.element_ty.is_block():
    ...
    # Build IR
    if mask is None:
        ret = tl.tensor(builder.create_load(ptr.handle, sem, scope, cache, eviction, is_volatile), dst_ty)
    else:
        ret = tl.tensor(
            builder.create_masked_load(ptr.handle, mask.handle, other.handle if other else None, sem, scope, cache,
...
def atomic_store(ptr: tl.tensor, val: tl.tensor, mask: Optional[tl.tensor], sem: str, scope: str, cache_modifier: str,
    ...
    if not mask.type.scalar.is_bool():
        raise ValueError("Mask must have boolean scalar type")
    return tl.tensor(builder.create_masked_store(ptr.handle, val.handle, mask.handle, sem, scope, cache, eviction),
                     tl.void)
