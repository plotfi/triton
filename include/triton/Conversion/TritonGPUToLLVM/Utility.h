...
emitBaseIndexForLayoutImpl(Location loc, RewriterBase &rewriter,
  ...
  if (auto blockedLayout = mlir::dyn_cast<BlockedEncodingAttr>(layout)) {
    ...
    return result;
  } else if (auto dotLayout = mlir::dyn_cast<DotOperandEncodingAttr>(layout)) {
...
emitOffsetForLayout(Attribute layout, RankedTensorType type) {
  ...
  if (auto dotLayout = mlir::dyn_cast<DotOperandEncodingAttr>(layout))
...
inline DenseMap<unsigned, Value> getSwizzledSharedPtrs(
  ...
  /// Hack swap
  if (!(outVec * maxPhase <= srcShape[outOrder[0]])) {
    auto tmp = outOrder[0];
    outOrder[0] = outOrder[1];
    outOrder[1] = tmp;
  }
...
gatherSharedToDistributed(Value dst, Value src, Value indices,
  ...
  assert(dstShape.size() <= 2 && "Unexpected rank of gatherSharedToDistributed");
...
inline void storeDistributedToShared(Value src, ArrayRef<Value> inVals,
  ...
  unsigned inVec = inOrd == outOrd
                       && !(mlir::isa<DotOperandEncodingAttr>(srcDistributedLayout))
                       ? triton::gpu::getUniqueContigPerThread(
